...
public class ExpressDynamicAdapter extends RecyclerView.Adapter<ExpressDynamicAdapter.ViewHolder> {
    ...
    public ExpressDynamicAdapter(Context context, List<ExpressDynamic> data) {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
        View itemView = LayoutInflater.from(mContext).inflate(R.layout.item_express_dynamic, viewGroup, false);
    ...
    public void onBindViewHolder(ViewHolder viewHolder, int i) {
        ...
        if (i == 0) {
            viewHolder.mItemTime.setTextColor(mContext.getColor(R.color.color_222));
            viewHolder.mItemDate.setTextColor(mContext.getColor(R.color.color_939393));
            viewHolder.mItemDynamic.setTextColor(mContext.getColor(R.color.color_494949));

            int radius = mContext.getResources().getDimensionPixelOffset(R.dimen.express_dynamic_dot_red_radius);
            ...
            int color = mContext.getColor(R.color.color_c2c2c2);
            viewHolder.mItemTime.setTextColor(color);
            viewHolder.mItemDate.setTextColor(color);
            viewHolder.mItemDynamic.setTextColor(color);

            int radius = mContext.getResources().getDimensionPixelOffset(R.dimen.express_dynamic_dot_gray_radius);
            viewHolder.mItemDot.setWidth(radius);
            viewHolder.mItemDot.setHeight(radius);
